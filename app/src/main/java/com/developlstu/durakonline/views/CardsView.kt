...
class CardsView : View {
    ...
    override fun onDraw(canvas: Canvas?) {
        ...
        if(value != null) {
            paint.apply {
                color = if (Suit.CLUBS == value?.suit || Suit.SPADES == value?.suit) Color.BLACK else Color.RED
                style = Paint.Style.FILL
                isAntiAlias = true
                textSize =  Math.min(widthCard,heightCard)*10/(25F)
            }
            canvas?.drawText("${value!!.value}${value!!.suit.suit}", (widthCard/130F)*10, (heightCard/50F)*10, paint)
            canvas?.drawText("${value!!.value}${value!!.suit.suit}", (widthCard/130F)*10, heightCard.toFloat() - 20F, paint)
